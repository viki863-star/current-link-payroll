...
def customer_invoice_pdf(cid, iid):
    ...
    if is_nmdc:
        ...
        hdr_cells = [
            ...
            Paragraph("<b>Total</b>", S("_h7", fontSize=fs, fontName="Helvetica-Bold", textColor=WH, alignment=TA_RIGHT, leading=ldr)),
        ]
        rws = [hdr_cells]
        for idx, it in enumerate(items):
            ...
            rws.append([
                ...
                Paragraph(f"<b>{ti_item:,.2f}</b>", S("_pt", fontSize=fs, fontName="Helvetica-Bold", leading=ldr, alignment=TA_RIGHT, textColor=C4)),
            ])

        itt = Table(rws, colWidths=cw_items, repeatRows=1)
        itt.setStyle(TableStyle([
            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
            ("BACKGROUND", (0, 0), (-1, 0), NAV), ("TEXTCOLOR", (0, 0), (-1, 0), WH),
            ("BOX", (0, 0), (-1, -1), 0.4, C3),
            ("INNERGRID", (0, 0), (-1, -1), 0.2, C3),
            ("TOPPADDING", (0, 0), (-1, -1), 3), ("BOTTOMPADDING", (0, 0), (-1, -1), 3),
        ...
        words_box.setStyle(TableStyle([
            ...
            ("SPAN", (0, 1), (-1, 1)), ("VALIGN", (0, 0), (0, 0), "MIDDLE"),
        ]))
        ...
        tot_rows = [
            ...
            [Paragraph("<b>Total Due</b>", S("_td", fontSize=9, fontName="Helvetica-Bold", textColor=WH, leading=13)),
             Paragraph(f"<b>AED {tot:,.2f}</b>", S("_tdv", fontSize=12, fontName="Helvetica-Bold", textColor=GOLD, leading=15, alignment=TA_RIGHT))],
        ]
        ...
        tt.setStyle(TableStyle([
            ...
            ("BACKGROUND", (0, 2), (-1, 2), NAV), ("BOX", (0, 0), (-1, -1), 0.4, C3),
        ]))

        bottom = Table([[words_box, Spacer(1, 4*mm), tt]], colWidths=[W2*0.55, 4*mm, tw])
        ...
        if display_notes:
            els2.append(Spacer(1, 3*mm))
            nb = Table([[Paragraph(f"<b>Notes:</b> {display_notes}", S("NW", fontSize=6.5, textColor=C4, leading=9))]], colWidths=[W2])
            nb.setStyle(TableStyle([
                ("BACKGROUND", (0, 0), (-1, -1), BG), ("BOX", (0, 0), (-1, -1), 0.4, C3),
                ("LEFTPADDING", (0, 0), (-1, -1), 4), ("RIGHTPADDING", (0, 0), (-1, -1), 4),
                ("TOPPADDING", (0, 0), (-1, -1), 2), ("BOTTOMPADDING", (0, 0), (-1, -1), 2),
            ]))
            els2.append(nb)
...
def customer_lpo_edit(cid, lid):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("customer.customer_profile", cid=cid, tab="lpos"))
    items = db.execute("SELECT id, lpo_id, description, quantity, rate, amount, created_at FROM lpo_items WHERE lpo_id=? ORDER BY sort_order", (lid,)).fetchall()
    db.close()
...
def customer_so_edit(cid, sid):
    ...
    if not c: return redirect(url_for("customer.customer_dashboard"))
    db = _get_db()
    so = db.execute("SELECT id, customer_id, so_no, so_date, amount, status, notes, created_at FROM customer_service_orders WHERE id=? AND customer_id=?", (sid, cid)).fetchone()
    if not so:
        db.close()
...
def customer_doc_download(cid, did):
    db = _get_db()
    doc = db.execute("SELECT id, customer_id, doc_type, doc_name, file_data, file_type, expiry_date, created_at FROM customer_documents WHERE id=? AND customer_id=?", (did, cid)).fetchone()
    db.close()
...
def customer_list():
    ...
    if search:
        customers = db.execute("SELECT id, customer_name, customer_code, contact_person, phone, email, address, trn, trade_license, credit_limit, payment_terms, status, notes, logo_data, logo_type, created_at FROM customers WHERE customer_name LIKE ? OR phone LIKE ? OR email LIKE ? ORDER BY customer_name",
            (f"%{search}%", f"%{search}%", f"%{search}%")).fetchall()
    else:
        customers = db.execute("SELECT id, customer_name, customer_code, contact_person, phone, email, address, trn, trade_license, credit_limit, payment_terms, status, notes, logo_data, logo_type, created_at FROM customers ORDER BY customer_name").fetchall()
    db.close()
...
def customer_tripsheet_edit(cid, tid):
    ...
    if not c: return redirect(url_for("customer.customer_dashboard"))
    db = _get_db()
    row = db.execute("SELECT id, customer_id, entry_date, time_in, time_out, total_reading, tanker_gln, trips, tanker_reg, bill_no, notes, created_at FROM tabreed_tripsheets WHERE id=? AND customer_id=?", (tid, cid)).fetchone()
    if not row:
        db.close()
