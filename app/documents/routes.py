...
def document_download(doc_id):
    db = open_db()
    doc = db.execute("SELECT id, entity_type, entity_id, doc_name, doc_category, issue_date, expiry_date, file_type, file_data, created_at FROM documents WHERE id=?", (doc_id,)).fetchone()
    db.close()
...
def document_edit(doc_id):
    db = open_db()
    doc = db.execute("SELECT id, entity_type, entity_id, doc_name, doc_category, issue_date, expiry_date, file_type, created_at FROM documents WHERE id=?", (doc_id,)).fetchone()
    if not doc:
        db.close()
        flash("Document not found.", "error")
        return redirect(url_for("documents.document_hub"))
